...
[`outrage.server.instructions()`](server.md#outrage.server.instructions) sends when a client connects -- so editing
it edits what every session is told, and `DELIVERY_BUDGET` in
[`outrage.server`](server.md#module-outrage.server) is what bounds it. The server reads that file directly
