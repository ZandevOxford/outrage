...
Arrange everything a project needs to use outrage: the MCP server entry in .mcp.json, in
.codex/config.toml for Codex, where it carries a marker so a re-run finds it, and in
.cursor/mcp.json for Cursor; a session-start hook for each harness - .claude/settings.json for
Claude Code, .github/hooks/outrage.json for Copilot CLI, .codex/hooks.json for Codex,
.cursor/hooks.json for Cursor - and the packaged skill and agents in .claude/, Copilot agents in
.github/agents/, and the Codex skills in .codex/. Cursor has no packaged files of its own. Only the
